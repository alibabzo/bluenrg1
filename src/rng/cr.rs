...
impl R {
    ...
    #[doc = "Bit 2 - Set the state of the random number generator.<ul><li>0: RNG is enable.</li><li>1: RNG is disabled. The internal free-running oscillators are put in power-down mode and the RNG clock is stopped at the input of the block.</li></ul>"]
    #[inline]
...
impl W {
    ...
    #[doc = "Bit 3 - RNG test clock bit. Writing this bit with 1b starts the logic that detects the presence of the CLK. Then wait (with a timeout of at least four RNGCLK cycles) for REVCLK = 1b in SR register. If REVCLK = 0b after timeout elapsed, it means that RNGCLK is not present and reading VAL register will trigger an AHB error response. For security reason, software should check before reading random values that the RNGCLK is present."]
    #[inline]
    pub fn tst_clk(&mut self) -> _TST_CLKW {
        _TST_CLKW { w: self }
    }
}
